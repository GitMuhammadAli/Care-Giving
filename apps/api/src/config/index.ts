...
import { optionalString, int, bool, optionalPassword, isDevelopment, isProduction } from './env.helpers';
...
export const redisConfig = registerAs('redis', () => {
  ...
  return {
    ...
    upstash: upstashRestUrl && upstashRestToken ? {
      url: upstashRestUrl,
...
export const jwtConfig = registerAs('jwt', () => ({
  ...
  refreshSecret: optionalString('JWT_REFRESH_SECRET', 'REPLACE_ME_MIN_32_CHARS'),
...
export const securityConfig = registerAs('security', () => ({
  encryptionKey: optionalString('ENCRYPTION_KEY', '0123456789abcdef0123456789abcdef'),
...
export const mailConfig = registerAs('mail', () => {
  ...
  const from = optionalString('MAIL_FROM') || optionalString('EMAIL_FROM', 'noreply@carecircle.com');
  const fromName = optionalString('MAIL_FROM_NAME') || optionalString('EMAIL_FROM_NAME', 'CareCircle');
...
export const pushConfig = registerAs('push', () => {
  ...
  const vapidSubject = optionalString('VAPID_SUBJECT', 'mailto:admin@carecircle.com');
...
export const firebaseConfig = registerAs('firebase', () => {
  const serviceAccountJson = optionalString('FIREBASE_SERVICE_ACCOUNT');

  return {
    serviceAccount: serviceAccountJson ? JSON.parse(serviceAccountJson) : null,
  };
});
