...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      "Creates a new user account. An email verification OTP will be sent to the provided email.",
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({
    ...
    description:
      "Verifies user email with the 6-digit OTP sent during registration. Returns tokens for automatic login.",
  })
  @ApiResponse({
    status: 200,
...
- Refresh token: 7 days (30 days with rememberMe)
  ...
  @ApiResponse({
    status: 200,
    description:
      "Login successful - copy the accessToken and use Authorize button",
  ...
  @ApiOperation({
    ...
    description:
      "Returns the authenticated user's profile including family memberships. Requires valid JWT token.",
  })
